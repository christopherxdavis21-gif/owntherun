...
export const Route = createFileRoute("/track")({
  head: () => ({
    meta: [
      { title: "Track a run — Catch Up" },
      {
        name: "description",
        content:
          "Track a free run live with GPS — no preset route required. Save it private, share it, or submit to the leaderboard.",
      },
    ],
  }),
  component: TrackPage,
...
function TrackPage() {
  ...
  const startedAtRef = useRef<number | null>(null); // ms timestamp of current segment start
  const accumulatedMsRef = useRef(0); // sum of completed segments
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        if (pos.coords.accuracy && pos.coords.accuracy > 30) {
          // still update center, but skip distance/path
          setCenter(next);
          return;
        }
        setCoords((prev) => {
          ...
          if (last) {
            ...
            // Ignore tiny movements (< 3m) — likely noise
            if (d < 3) return prev;
  ...
  const handleStop = () => {
    ...
    setStatus("stopped");
    if (coords.length > 0) {
      // Suggest a default route name from the start time
      const now = new Date();
      setRouteName(
  ...
  const save = async () => {
    ...
    try {
      ...
      // Optionally compute elevation gain for context
      let elevationGain = 0;
      try {
        const elev = await computeElevationGain({ data: { coordinates: coords } });
        elevationGain = elev.elevation_gain_meters;
      } catch {
        // Non-fatal
      }

      // Optionally save the traced path as a route
  ...
  return (
    <AppShell>
      <div className="mb-6">
        <p className="eyebrow text-primary">Track a run</p>
        <h1 className="font-display text-4xl font-black tracking-tight">
          Just go. We'll log it.
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Hit start and run wherever you want — no route required. We'll record your
          path, distance, time, and pace from your device's GPS.
        </p>
      </div>
            ...
            <>
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Choose how this run is shared. You can also save the path you ran
                  as a reusable route.
                </p>
              </div>
